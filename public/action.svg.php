...
$profileHost = strtoupper((string)(parse_url($brand['profile_url'], PHP_URL_HOST) ?: 'GITHUB'));
$profilePath = strtoupper(trim((string)(parse_url($brand['profile_url'], PHP_URL_PATH) ?: ''), '/'));
$detail = match ($action) { 'repo' => 'SOURCE / README', 'profile' => $profileHost . ($profilePath !== '' ? ' / ' . $profilePath : ''), default => 'LEAVE THE NEXT INK' };
...
<svg xmlns="http://www.w3.org/2000/svg" width="420" height="76" viewBox="0 0 420 76" role="img" aria-label="<?= $label ?>">
  ...
  <circle cx="26" cy="27" r="5" fill="<?= $brand['accent'] ?>"/>
