...
<?php foreach ($notes as $note): ?><article class="note"><div><h3><?= iw_e($note['author_name']) ?> <span class="pill <?= iw_e($note['status']) ?>"><?= iw_e($note['status']) ?></span></h3><p><?= nl2br(iw_e($note['message_text'])) ?></p></div><div class="meta">ID <?= iw_e($note['id']) ?><br><?= iw_e($note['created_at']) ?><br><a href="?visitor=<?= iw_e(iw_short_hash($note['visitor_hash'])) ?>">user <?= iw_e(iw_short_hash($note['visitor_hash'])) ?>…</a> · <?= iw_e($note['country']) ?> · <?= iw_e($note['referrer_host']) ?><br><?= (int)$note['image_bytes'] ?> image bytes · <?= (int)$note['reaction_count'] ?> reactions · <?= (int)$note['report_count'] ?> reports</div><div class="actions">
